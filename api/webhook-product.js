...
function isZakekeProduct(p) {
  ...
  return /zakeke/i.test(pt); // couvre provider-zakeke-product ET zakeke-design
}
...
async function updateProduct(gid, tag, rename = true) {
  const pd = await getProduct(gid);
  if (!pd) throw new Error('Produit introuvable');
  const tags = Array.from(new Set([...(pd.tags || []), tag]));
  let title = pd.title;
  if (rename) {
    const comp = companyFromProTag(tag);
...
module.exports = async function handler(req, res) {
  try {
    ...
    const designId = null; // si tu n'as pas un champ sûr à parser, on s'appuie sur la session
    ...
    console.log('[webhook-product] tagged/renamed', { productId, tag: attr.customerTag, title: updated.title });
